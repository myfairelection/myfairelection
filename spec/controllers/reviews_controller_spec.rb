...
describe ReviewsController do
  ...
  describe 'POST create' do
    context "without a polling location set" do
      let(:params) { 
        { "review" => 
          { "voted_day" => "11-06",
            "voted_time" => "13:45",
      ...
      it "asks for a list of polling locations near the user" do
        PollingLocation.should_receive(:find_near).with(include("zip", "80014"))
        post 'create', params
      end
      it "passes the list of locations to the view" do
        post 'create', params
        expect(assigns[:polling_locations]).to_not be_nil
      end
    end
    context "with a polling location set" do
      let(:params) { 
        { "review" => 
          { "voted_day" => "11-06",
            "voted_time" => "13:45",
