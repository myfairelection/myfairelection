  ...
  describe "POST 'create'" do
    context "with a logged in user" do
      ...
      context "with valid params" do
        let (:params) {
          { polling_location:
            { state: "CA", description: "I voted in a shoebox at 3rd and 16th" }
          }
        }
        it "creates a new polling location" do
          expect {post 'create', params}.to change{PollingLocation.count}.by(1)
        end
        it "redirects to the create review page for the new location" do
          post 'create', params
          expect(response).to redirect_to new_polling_location_review_path(assigns[:polling_location])
        end
